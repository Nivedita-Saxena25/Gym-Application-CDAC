...
import java.util.Optional;
	...
	public class UserService  implements IUsersService,UserDetailsService {
		...
		public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
			return usersDAO.findByUsername(username).orElseThrow(()->new NotFoundException("username is not found"));
		}
...
public class UserService implements IUsersService {
	...
	@Autowired
    private final IFeedbackAndSurveys feedbackAndSurveyRepository;
    ...
    public UserService(
            ...
            IHealthAndFitnessTracking healthTrackingRepository,
            IBooking bookingRepository,
            IAttendance attendanceRepository,
            IDietPlan dietPlanRepository,
            IClassSchedule classScheduleRepository,
            IFeedbackAndSurveys feedbackAndSurveyRepository) {
        ...
        this.feedbackAndSurveyRepository = feedbackAndSurveyRepository;
    }
    ...
    public Users updateUser(int id, Users updatedUser) {
        ...
        if (optionalUser.isPresent()) {
            ...
            existingUser.setFirstName(updatedUser.getFirstName() != null && !updatedUser.getFirstName().isEmpty()
                    ? updatedUser.getFirstName() : existingUser.getFirstName());
            existingUser.setLastName(updatedUser.getLastName() != null && !updatedUser.getLastName().isEmpty()
                    ? updatedUser.getLastName() : existingUser.getLastName());
            existingUser.setAddress(updatedUser.getAddress() != null && !updatedUser.getAddress().isEmpty()
                    ? updatedUser.getAddress() : existingUser.getAddress());
            existingUser.setContactNumber(updatedUser.getContactNumber() != null && !updatedUser.getContactNumber().isEmpty()
                    ? updatedUser.getContactNumber() : existingUser.getContactNumber());
            existingUser.setEmail(updatedUser.getEmail() != null && !updatedUser.getEmail().isEmpty()
                    ? updatedUser.getEmail() : existingUser.getEmail());
            existingUser.setHealthRecords(updatedUser.getHealthRecords() != null && !updatedUser.getHealthRecords().isEmpty()
                    ? updatedUser.getHealthRecords() : existingUser.getHealthRecords());
            existingUser.setRegistrationDate(updatedUser.getRegistrationDate() != null
                    ? updatedUser.getRegistrationDate() : existingUser.getRegistrationDate());
	...
	public void saveUserAndLoginWithOtherEntities(Users user, Login login, HealthAndFitnessTracking healthTracking,
			DietPlan dietPlan, FeedbackAndSurveys feedbackAndSurvey, Booking booking, Attendance attendance,
        ...
        // Save FeedbackAndSurveys
        feedbackAndSurvey.setUser(savedUser);
        feedbackAndSurveyRepository.save(feedbackAndSurvey);
